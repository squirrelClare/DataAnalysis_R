...
test<-function(id) {
  ...
  preDate<-'2013-10-01'
  wethData<-read.csv(file = '~/Desktop/南山区_西丽.csv')
  loadData<-holiday_exeDm2015(paste0("select * from JL_TAIQU_LOAD where VKONT='",id,"';"))[4:100]
#   loadDataFilt<-holiday_loadDataFilter(loadData,k_size = 3)
  ...
  ratio<-rep(0,100)
  for (i in 1:100) {
    loadDataPredic<-holiday_prediction(0,loadData,wethData,'2013-10-01',ahead = 3,0.17,0.01,i/100)
    ratio[i]<-holiday_errorStatic(loadData,loadDataPredic,preDate)
  }
  print(which(ratio==max(ratio)))
  print(max(ratio))
}
holiday_errorStatic<- function (loadData,loadDataPredic,preDate) {
  ...
  realLoad<-as.matrix.data.frame(tmp[2:97])
  rownames(realLoad)<-tmp$LOAD_DATE
  
  tmpPredic<-as.matrix.data.frame(loadDataPredic[2:97])
  rownames(tmpPredic)<-loadDataPredic$LOAD_DATE
  
  d_1<-abs(realLoad-tmpPredic)/realLoad
...
#   t_1<-prop.table(cut(d_1, breaks = c(0, .05, .1, .15, .2, .4, 1)))
#   return (sum (t_1[1:2]))
# }
