...
viewId<-function(id,wethData,preDate,ahead) {
	...
	preds=holiday_prediction(models,loadData,wethData,preDate,ahead,alpha = .1,beta_1 = .90,beta_2 = .90)
...
test<-function() {
	...
	sql<-'select ID from holiday_forecast_result;'
	ids<-holiday_exeHumbird(sql)$ID
