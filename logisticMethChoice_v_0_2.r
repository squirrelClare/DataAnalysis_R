...
#获取与负荷相关的参数
logisticMethChoice_loadInf <- function(preDate, loadData, ahead) {
	#基础负荷信息
...
logisticMethChoice_generatTable <- function(tableName) {
	...
	for (vkont in unique(ratioData$id)) {		
		...
		for (j in 1:7) {
			#气温参数
			wethInf <- logisticMethChoice_wethInf(as.character(as.Date(preDate)+j-1), wethData)
			loadInf <- logisticMethChoice_loadInf(as.character(as.Date(preDate)+j-1), loadData, j)
			# tmp <- c(id=vkont)
...
logisticMethChoice_modelTrain <- function() {
  srcData1 <- read.csv('E:/新特征集/wthCycle/wthCycle20140311_preds_new.csv')
  srcData2 <- read.csv('E:/新特征集/wthCycle/wthCycle20140411_preds_new.csv')
...
holiday_dateCorr<-function(sameTypeDays,preDate,beta_1=.90) {
