...
predictionStatic_staticSavePerModel <- function(realLoadTableName, preLoadTableName, fIndex, tIndex, dstTableName) {
	print('START!!!')

	sql <- paste0("select distinct(forecast_date) from ",preLoadTableName,";")
	forecastDate <- as.Date(unlist(predictionStatic_exeLoadmonitoring(sql)))   #获取预测表中日期
	...
	#判断选取的公共编号是否越界
	if (length(idsOfIntersect) < tIndex) {
		tIndex <- length(idsOfIntersect)
	}

	predictionStatic_errorRatioComputerAndSave(fDate, tDate, idsOfIntersect[fIndex:tIndex], 
			realLoadTableName, preLoadTableName, dstTableName)
	print('SUCCESS!!!')
}
#计算并保存误差率
predictionStatic_errorRatioComputerAndSave <- function(fDate, tDate, idsOfIntersect, 
		realLoadTableName, preLoadTableName, dstTableName) {
	...
	errorRatioDataFrame <- rbind(errorRatioDataFrame, 
															 predictionStatic_getMeanErroRatioOfPartId(fDate, tDate, ids, realLoadTableName, preLoadTableName))
	write.csv(x = errorRatioDataFrame, file = dstTableName)
	print(paste0(dstTableName,"保存成功！！！！"))
}
...
predictionStatic_getMeanErroRatioOfPartId <- function(fDate, tDate, ids, realLoadTableName, preLoadTableName) {
	...
	preLoadData <- predictionStatic_getPredictLoadData(fDate, tDate, ids, preLoadTableName)	#获取预测负荷值

	ratioDataFrame <- predictionStatic_getMeanErroRatioOfPerId(srcLoadData, preLoadData, fDate, tDate, ids[1])

	for (id in ids[-1]) {
		ratioDataFrame <- rbind(ratioDataFrame,predictionStatic_getMeanErroRatioOfPerId(srcLoadData, preLoadData, fDate, tDate, id))
	}
...
predictionStatic_main <- function() {
	predictionStatic_staticSavePerModel(realLoadTableName = "loadmonitoring_load_realtime_201507", 
		preLoadTableName = 'loadmonitoring_forecast_result', fIndex = 40001, 
		tIndex = 50000, dstTableName = "F:/f40001to50000.csv")
}
