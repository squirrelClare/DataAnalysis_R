...
#     View(temp_hist)
#     print(temp_hist$WETH_DATE)
#     print(weekdays(as.Date(temp_hist$WETH_DATE)))
		...
		tmp_pre_l_weth<-t(data$h_weather_l_year[i,][c('MIN_TMP','MAX_TMP')])
	    	#系数计算结果
	    	date_and_coef<-holiday_globalCoef(temp_hist,tmp_pre_t_weth,temp_hist$WETH_DATE,tmp_pre_t_date,n_top =4,alpha,beta_1,beta_2)
...
viewId<-function(id,wethData) {
	...
	preds=holiday_prediction(models,loadData,wethData,predate="2013-10-1",ahead=3,alpha = .8,beta_1 = .90,beta_2 = .90)
...
#   res<-sapply(ids[1:3],FUN = viewId)
  	

  	res<-data.frame()
  	for(id in ids) {
    		res<-rbind(res,viewId(id,wethData))
  	}
	...
	conn<-dbConnect(RMySQL::MySQL(),dbname='DM2015',username='dimer',host='192.168.10.87',
		port=3306,password='data123')
	dbSendQuery(conn,statement="set names 'utf8';")
  dbWriteTable(conn,name = 'holiday_20131001_1007',res)
	dbDisconnect(conn)
}
# loadData$X=NULL
# loadData$LOAD_DATE=as.Date(loadData$LOAD_DATE)
