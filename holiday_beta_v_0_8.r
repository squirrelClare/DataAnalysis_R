...
holiday_prediction<-function(model,loadData,wethData,predate,ahead,alpha=.8,beta_1=-.95,beta_2=.92)
{	
  ...
  #判断所需负荷数据是否完整，当数据不完整时返回NULL并推出
	if (nrow(data$h_load_t_year)==0 |nrow(data$h_load_l_year)==0
  		|nrow(data$pr_h_load_t_year)==0|nrow(data$pr_h_load_l_year)==0) {
		return(NULL)
	}
	...
	for (i in 1:ahead) {
		...
		tmp_pre_l_weth<-t(data$h_weather_l_year[i,][c('MIN_TMP','MAX_TMP')])
	    	#系数计算结果
	    	date_and_coef<-holiday_globalCoef(temp_hist,tmp_pre_t_weth,temp_hist$WETH_DATE,tmp_pre_t_date,n_top =6,alpha,beta_1,beta_2)
	    	date_and_coef<-date_and_coef/sum(date_and_coef)

	    	# print(date_and_coef)
	    	# print(weekdays(as.Date(names(date_and_coef))))
	    	# print("**********&&&&&&&&&&&&***************%%%%$$$$$")
	    	...
	    	for(rname in rownames(sim_load)){
		    	if(as.Date(rname)==as.Date(tmp_pre_l_date)){
		    		sim_load[rname,]=sim_load[rname,]*as.numeric(date_and_coef[rname])
		    	}
		    	else{
		    		sim_load[rname,]=sim_load[rname,]*as.numeric(date_and_coef[rname])
		    	}
...
holiday_dataSplit<-function(loadData,wethData,predate,ahead){
	...
	pr_h_date_t_year<-intersect(pr_h_date_t_year,as.Date(loadData$LOAD_DATE))
	#预测日期去年同期前60天中为周末的日期
	pr_h_date_l_year<-holiday_weekday(as.Date(-60,origin =same_day_l),as.Date(-1,origin =same_day_l))
	pr_h_date_l_year<-intersect(pr_h_date_l_year,as.Date(loadData$LOAD_DATE))
	#被预测的日期负荷
...
#   daynums[which(week==weekeys[1])]=0.8
#   daynums[which(week==weekeys[4])]=1.8
#   daynums[which(week==weekeys[5])]=0.5
#   daynums[which(week==weekeys[6])]=0.8
#   daynums[which(week==weekeys[2])]=0.6
#   daynums[which(week==weekeys[3])]=1.5
#   return(daynums)
# }
...
holiday_nnmiss<-function (x, xmiss, ismiss, K) 
{
  ...
  holiday_plt<-function(preds, id) {
  	...
  	path<-paste0('~/Desktop/preds&src/')
  	if (!file.exists(path)){
  		dir.create(path)
  	}
  	setwd(path)
  	for(i in 1:nrow(preds)){
  		tiff(file=paste0(id,'_',tmp_dates[i],'.tiff'), res = 300, width = 2400, height = 2400, compression = "lzw")    
    	#画图
    	# rang<-c(min(min(preds[i,]),min(loadData[i,])),max(max(preds[i,]),max(loadData[i,])))+c(-50,50)

	plot(as.numeric(preds[i,]),type='l',col = 'red',xaxt = 'n',xlim = c(0,100),xlab = '时点',ylab = '负荷')
...
viewId<-function(id,wethData,preDate,ahead) {
	sql<-paste0("select * from JL_TAIQU_LOAD where VKONT='",id,"' order by LOAD_DATE;")
	loadData<-holiday_exeDm2015(sql)[4:100]
	...
	preds=holiday_prediction(models,loadData,wethData,preDate,ahead,alpha = .24,beta_1 = .02,beta_2 = .92)
# 	holiday_plt(preds,id)
  	#如果预测结果为NULL，则返回NULL
	if(is.null(preds)) {
		return(NULL)
	}
...
test<-function() {
	...
	sql<-'select VKONT from futian_ids;'
	ids<-holiday_exeDm2015(sql)$VKONT

	preDates<-c('2013-10-1','2013-10-4','2014-1-1','2014-1-31','2014-2-3','2014-4-5','2014-5-1','2014-6-2','2014-9-8')
	aheads<-c(3,4,3,3,4,3,3,3,1)
	tNames<-c('holiday_futian1_20131001_1003','holiday_futian1_20131004_1007','holiday_futian1_20140101_0103','holiday_futian1_20140131_0202','holiday_futian1_20140203_0206',
	          'holiday_futian1_20140405_0407','holiday_futian1_20140501_0503','holiday_futian1_20140602_0605','holiday_futian1_20140908')
	for (i in 1:9) {
		res<-data.frame()
		for(id in ids) {
			print(id)
			tmpRes<-viewId(id,wethData,preDates[i],aheads[i])
			if(!is.null(tmpRes)) {
		...
		conn<-dbConnect(RMySQL::MySQL(),dbname='DM2015',username='dimer',host='192.168.10.87',
			port=3306,password='data123')
		dbSendQuery(conn,statement="set names 'utf8';")
		dbWriteTable(conn,name = tNames[i],res)
		dbDisconnect(conn)
	}
}
# loadData$X=NULL
...
# holiday_init()
# models=holiday_buildModel(loadData,wethData,startdate="2012-09-01",enddate="2014-08-20")
# preds=holiday_prediction(models,loadData,wethData,predate="2014-9-2",ahead=7,alpha = .5,beta_1 = .5,beta_2 = .3)
# holiday_plt(preds,id = '000047001677')
